...
class WaitCore : public util::IRef {
 public:
  ...
  void Wait(std::unique_lock<yaclib_std::mutex>& guard);

  template <typename Rep, typename Period>
  bool Wait(std::unique_lock<yaclib_std::mutex>& guard, const std::chrono::duration<Rep, Period>& timeout_duration) {
  ...
  bool Wait(std::unique_lock<yaclib_std::mutex>& guard, const std::chrono::time_point<Clock, Duration>& timeout_time) {
