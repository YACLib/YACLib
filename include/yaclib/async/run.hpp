...
YACLIB_INLINE auto Run(IExecutor& e, Func&& f) {
  auto* core = [&] {
    if constexpr (std::is_same_v<V, Unit>) {
      ...
      return MakeCore<CoreT, void, T>(std::forward<Func>(f));
    } else {
      return MakeUnique<PromiseCore<V, T, Func&&, false>>(std::forward<Func>(f)).Release();
    }
  }();
  e.IncRef();
...
YACLIB_INLINE auto RunShared(IExecutor& e, Func&& f) {
  auto* core = [&] {
    if constexpr (std::is_same_v<V, Unit>) {
      ...
      return MakeShared<PromiseCore<V, T, Func&&, true>>(detail::kSharedRefWithFuture, std::forward<Func>(f)).Release();
    }
  }();
  e.IncRef();
  core->_executor.Reset(NoRefTag{}, &e);
...
/*SharedFuture*/ auto RunShared(Func&& f) {
  return detail::RunShared<Unit, T>(MakeInline(), std::forward<Func>(f)).On(nullptr);
}
...
/*SharedFutureOn*/ auto RunShared(IExecutor& e, Func&& f) {
  return detail::RunShared<Unit, T>(e, std::forward<Func>(f));
}
...
/*SharedFuture*/ auto AsyncSharedContract(Func&& f) {
  return detail::RunShared<V, T>(MakeInline(), std::forward<Func>(f));
}
...
/*SharedFuture*/ auto AsyncSharedContract(IExecutor& e, Func&& f) {
  return detail::RunShared<V, T>(e, std::forward<Func>(f));
}
