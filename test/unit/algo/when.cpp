...
TYPED_TEST(WhenSuite, UniqueStatic) {
  ...
  auto f = yaclib::detail::When<typename TestFixture::Strategy>(std::move(f1), std::move(f2), std::move(f3));
...
TYPED_TEST(WhenSuite, UniqueDynamic) {
  if constexpr (TestFixture::Strategy::Order == yaclib::StrategyOrder::Static) {
    ...
    auto f = yaclib::detail::When<typename TestFixture::Strategy>(vec.begin(), vec.size());
    EXPECT_EQ(std::move(f).Get().Value(), (yaclib::Unit{}));
  }
}
...
TYPED_TEST(WhenSuite, AsyncStatic) {
  ...
  auto f =
    yaclib::detail::When<typename TestFixture::Strategy>(std::move(f1), std::move(f2), std::move(f3), std::move(f4));
  EXPECT_EQ(std::move(f).Get().Value(), (yaclib::Unit{}));
